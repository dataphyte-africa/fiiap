...
export function CreatePostModal({
            ...
            if (event.target.files) {
                ...
                selectedFiles.forEach((file) => {
                    ...
                    if (!ALLOWED_FILE_TYPES.includes(file.type)) {
                        errors.push(
                            `${file.name}: File type not allowed. Only images and videos are supported.`
                        );
                        return;
                    }
    ...
    const getFilePreview = (
        file: File | undefined,
        remoteFile?: { url: string; type: string }
    ) => {
        if (remoteFile) {
    ...
    const handleSubmit = async () => {
        ...
        if (removedMedia.length) {
            const promises = removedMedia.map((file) =>
                storage.deleteFile(
                    STORAGE_BUCKETS.ORGANISATION_MEDIA,
                    file.storage_path || ''
                )
            );
            const mediaPromises = removedMedia.map((file) =>
                removeForumMedia(file.id)
            );
        ...
        try {
            ...
            if (profile?.organisation_id) {
                ...
                queryClient.invalidateQueries({
                    queryKey: ['posts', profile.organisation_id],
                });
            }
            ...
            if (profile?.organisation_id) {
                console.log(profile.organisation_id, '🌹 invalidate posts');
                queryClient.invalidateQueries({
                    queryKey: ['posts', profile.organisation_id],
                });
            }
    ...
    const handleCreatePost = async (
        uploadResults: StorageUploadResult['data'][]
    ) => {
        // Create the post
    ...
    const removeForumMedia = async (mediaId: string) => {
        const { error: mediaError } = await supabase
            .from('forum_media')
            .delete()
            .eq('id', mediaId);
                    ...
                    {(selectedFiles.length > 0 || media.length > 0) && (
                        <div className="space-y-2">
                            <div className="flex items-center justify-between">
                                <Label>
                                    Media Attachments ({selectedFiles.length})
                                </Label>
                                <Button
                                    variant="outline"
                                    size="sm"
