...
import { OrganisationStatusBadge } from '../organisations/organisation-status-badge'
import { updateOrganisationStatus, toggleOrganisationVerification } from '@/lib/data/organisations'
import type { Database } from '@/types/db'
import { useRouter } from 'next/navigation'

type OrganisationStatus = Database['public']['Enums']['organisation_status_enum']

interface AdminOrganisationViewProps {
  organisation: Organisation
  className?: string
}

export function AdminOrganisationView({ 
  organisation, 
  className 
}: AdminOrganisationViewProps) {
  ...
  const handleStatusUpdate = async (newStatus: OrganisationStatus) => {
    setActionLoading(prev => new Set(prev).add(organisation.id))
    try {
  ...
  const handleVerificationToggle = async () => {
    setActionLoading(prev => new Set(prev).add(organisation.id))
    try {
      const result = await toggleOrganisationVerification(organisation.id, !organisation.verified)
      if (result.success) {
        // Update local state
                ...
                <div className="flex flex-col sm:items-end gap-3">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <Button 
                      ...
                      disabled={actionLoading.has(organisation.id)}
                      variant={organisation.verified ? "destructive" : "default"}
                      size="sm"
                    >
